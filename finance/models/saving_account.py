...
class SavingAccount(BaseModel):
    ...
    balances = models.ManyToManyField(SavingAccountBalance, blank=True)
    deposits = models.ManyToManyField(SavingAccountDeposit, blank=True)
    ...
    def get_balance(self, max_date: datetime.datetime | None = None) -> Money:
        ...
        current_balance = (
            self.balances.filter(balance_date__lte=max_date)
