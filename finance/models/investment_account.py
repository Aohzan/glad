...
class InvestmentAccount(BaseModel):
    ...
    cash_balances = models.ManyToManyField(
    ...
    def get_value(self, max_date: datetime.datetime | None = None) -> Money:
        ...
        if (
            current_balance := self.cash_balances.filter(balance_date__lte=max_date)
        ...
        if not holdings.exists():
            return Money(cash_balance.balance.amount, self.currency)
